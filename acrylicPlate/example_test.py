...
def test_examples_protocol_https_request(env, extra_data):
    ...
    steps: |
      ...
      3. send http request
    """
    dut1 = env.get_dut("acrylicPlate", "examples/protocols/acrylicPlate")
    # check and log bin size
    binary_file = os.path.join(dut1.app.binary_path, "acrylicPlate.bin")
    bin_size = os.path.getsize(binary_file)
    IDF.log_performance("acrylicPlate_bin_size", "{}KB".format(bin_size // 1024))
    IDF.check_performance("acrylicPlate_bin_size", bin_size // 1024)
    # start test
    dut1.start_app()
